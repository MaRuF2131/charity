...
const UpdatePostModal = ({ onClose }) => {
  const location = useLocation();
  const{logout}=useAuthContext();
  const post = location.state?.value;
  const navigate=useNavigate()
      ...
      useEffect(() => {
      document.title ="Update post - Charity"; // your dynamic title
    }, []);
  ...
  const [deadline, setDeadline] = useState(new Date(post?.deadline || new Date()));
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const updatedPost = { ...data, deadline };

      await axiosInstance.patch(`/api/private/update-volunteer/${post._id}`, updatedPost)
      .then((res)=>{
          Swal.fire("Updated!", "Post updated successfully.", "success").
          then((result)=>{
              if(result.isConfirmed){navigate('/my-post')}
          }
          );
         }
        ).catch(err=>{Unauthorize(logout,err,"Update failed.")});
    } catch (err) {
      Unauthorize(logout,err,"Update failed.")
      console.error(err);
    }
  };
  ...
  return (
     <div className="min-h-screen  py-16 px-4 flex items-center justify-center">
           ...
           >
             ...
             <div className="absolute inset-0 rounded-3xl bg-gradient-to-br from-indigo-200 via-purple-200 to-pink-200  opacity-20  dark:bg-gray-500 dark:bg-none blur-3xl z-[-1]" />
             ...
             <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 md:grid-cols-2 gap-8 text-sm md:text-base">
               ...
               <div>
                 ...
                 <input value={post?.organizerName} readOnly className="input input-bordered w-full bg-gray-100 shadow-sm" />
               </div>
     
               <div>
                 <label className="font-semibold mb-1 block">Organizer Email</label>
                 <input value={post?.organizerEmail} readOnly className="input input-bordered w-full bg-gray-100 shadow-sm" />
               </div>
